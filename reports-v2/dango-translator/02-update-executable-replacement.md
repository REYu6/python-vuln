# Dango-Translator: Unauthenticated update metadata permits executable replacement

**Upstream issue:** [PantsuDango/Dango-Translator#187](https://github.com/PantsuDango/Dango-Translator/issues/187)
...
Weakness: CWE-494 / CWE-295

## Description

The updater trusts ShowDict metadata fetched with TLS verification disabled and installs downloaded bytes without an independent signature. The main program path is replaced after update confirmation; `utils/update.py:41-60` can also replace the updater itself using attacker-supplied URL/MD5 metadata without that confirmation.

## Impact

A network-path attacker can replace installed executable files, creating a code-execution path when a valid malicious executable is launched. The test proves replacement only: inert non-PE marker files intentionally failed to execute.
...
For non-update tests keep `latest_version` at 4.5.8 in state.json and leave updater MD5/URL overrides unset. The update test uses the supplied Run2 example, followed by Run3 with `auto_update_file_md5` set to 32 zeros and `auto_update_file_url` pointing to the local updater-marker endpoint. The shared harness has additional stages; only the finding-specific stages below are necessary.

Run2: place the JSON object from state-run2.example.txt in `mitm/state.json` (omit its leading comment), run `launch`, then `update-click` to confirm the dialog. Run3: restore the genuine updater first in the disposable fixture, set the updater MD5 override to 32 zeros and the updater URL to `http://127.0.0.1:18443/payload/autoupdate.exe`, then relaunch; no update confirmation is needed for that replacement.
...
### `mitm/state-run2.example.txt`

```json
// mitm/state.json — 假配置中心按本文件动态生成 ShowDict 的 Result (Run2)
{"latest_version": "9.9.9", "update_url": "http://127.0.0.1:18443/payload/main_update.exe",
 "auto_update_file_md5": "", "auto_update_file_url": "http://127.0.0.1:18443/payload/autoupdate.exe"}
```
...
The genuine updater replaced a 175-byte placeholder main executable with 1,480 marker bytes after the GUI confirmation. A separate run silently replaced the genuine 7,764,277-byte updater with 1,520 marker bytes. No malicious PE execution or working RCE payload was tested.
...
```text
[evidence/mitm_server.log]
[16:47:59] 443 POST /DangoTranslate/ShowDict        ← app 拉取伪造 dict_info (UA python-requests/2.23.0)
[16:48:46] 443 POST /DangoTranslate/ShowDict        ← 自动更新程序.exe (UA python-requests/2.26.0), 自签证书被 verify=False 接受
[16:48:46] 18443 GET /payload/main_update.exe → 1480B 标记载荷被下载
[evidence/main_program_diff.txt] 团子翻译器.exe 字节 diff:
  before sha256=eb10460b...(175B 占位主程序) → after sha256=a61473c8...(1480B, DANGO-FULLSTACK-PWN-42bd86232c97c8bf x40)
  即 update.py:121 的 shutil.move 在真机完成主程序替换 (os.startfile 因载荷非 PE 失败, 属预期, 载荷不执行)
静默替换(Run3 零点击): 自动更新程序.exe sha256 de2f9698...(7,764,277B 真更新器) → 77af73e0...(1520B DANGO-AUTOUPDATE-PWN)
```

## Suggested fix

Authenticate update metadata and binaries with a separately trusted signing key; verify before atomic replacement and preserve rollback. Enable TLS verification, restrict update origins, and avoid trusting hashes received alongside attacker-controlled URLs.
