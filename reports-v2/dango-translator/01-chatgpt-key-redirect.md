# Dango-Translator: Unverified remote settings can redirect a private ChatGPT Bearer key

**Upstream issue:** [PantsuDango/Dango-Translator#186](https://github.com/PantsuDango/Dango-Translator/issues/186)
...
Weakness: CWE-295 / CWE-200

## Description

`utils/http.py:26-28` disables TLS certificate verification for remote settings. `chatgptApiAddr` is accepted from those settings, and `translator/api.py:298,332-354` sends the user's Bearer key to that URL without destination validation.

## Impact

A network-path attacker who forges settings can receive the configured private API key when the user tests/uses that integration. Unauthorized API usage is a possible consequence; the reproduction used a fake key and incurred no demonstrated billing loss.
...
Stages: `gui_driver.py launch`, `login`, then `chatgpt`. Before `chatgpt`, open the Settings window manually: the preserved `stage_chatgpt()` function requires an already-open `Settin` window. This precondition is visible in the supplied driver; it was not retested during report preparation. The private ChatGPT settings dialog displays the injected collector URL; pressing its Test button sends the synthetic key.
...
The real GUI accepted the poisoned URL and sent `Authorization: Bearer sk-DANGO-MITM-FAKE-KEY-42bd86232c97c8bf` to the loopback collector, with a 2,017-byte request body. The configuration was subsequently saved through the forged settings service.
...
```text
[evidence/captures/18090_*steal_chatgpt*.json]
time: 2026-09-21 16:32:24 | path: /steal/chatgpt | body 2017B
Authorization: Bearer sk-DANGO-MITM-FAKE-KEY-42bd86232c97c8bf
[16:33:27] 443 POST /DangoTranslate/SaveSettin body=5022  ← 含毒化地址+假密钥的配置被 app 回存攻击者服务器
```

## Suggested fix

Enable normal TLS verification. Do not let remotely supplied settings silently change destinations for user-managed credentials; require explicit approval and bind credentials to the approved HTTPS destination.
