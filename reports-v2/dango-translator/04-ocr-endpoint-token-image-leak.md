# Dango-Translator: Unverified OCR endpoint metadata exposes account tokens and image contents

**Upstream issue:** [PantsuDango/Dango-Translator#189](https://github.com/PantsuDango/Dango-Translator/issues/189)
...
ShowDict metadata is fetched without TLS verification and supplies OCR/translation endpoint URLs. `translator/ocr/dango.py:363-466` trusts these destinations and sends token-bearing requests containing image data.

## Impact

A network-path attacker can redirect subsequent OCR/translation operations to obtain the account token and submitted images. Account abuse depends on the token's server-side privileges; no real account was compromised in the test.
...
Stages: `launch`, `login`, `ocrtest`, then `manga`. Inspect the collector captures for the synthetic token and image body.
...
The real GUI sent a synthetic token and image data to the loopback collector during OCR Test and manga translation. Captured requests included OCR (39,972 bytes), manga OCR (829 bytes), translation (104 bytes), inpainting (791 bytes), and rendering (787 bytes).
...
```text
[evidence/captures/18090_019__ocr_api.json 尾部] ..."Verify": "Token", "Token": "DANGO-FAKE-TOKEN-42bd86232c97c8bf"}  (body 39972B 含图)
[16:46:42] 18090 POST /v2/manga_trans/advanced/manga_ocr    body=829  [含假token+整图]
[16:46:42] 18090 POST /v2/translate/sync_task     body=104  {"token": "DANGO-FAKE-TOKEN-...", "texts": ["MITM-PWNED-TEXT"], "from": "JAP", "to": "CHS"}
[16:46:42] 18090 POST /v2/manga_trans/advanced/text_inpaint body=791 [含假token]
[16:46:42] 18090 POST /v2/manga_trans/advanced/text_render  body=787 [含假token]
[16:37:59] 18090 POST /check_permission?Token=DANGO-FAKE-TOKEN-... /ocr/query_quota?Token=...  (token 直现 URL)
```

## Suggested fix

Verify TLS certificates for metadata and service requests. Bind tokens to approved HTTPS origins and validate remote endpoint configuration before transmitting credentials or image contents.
